...
namespace Vireo {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			VIR_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			VIR_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			VIR_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");

...
namespace Vireo {
	...
	Editor3DLayer::Editor3DLayer()
		: Layer("Editor3DLayer"), m_CameraController(1280.0f / 720.0f), m_SquareColor({ 0.2f, 0.3f, 0.8f, 1.0f })
	{
	}
	...
	void Editor3DLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
			{
				...
				std::filesystem::path droppedPath = std::filesystem::path(g_AssetPath) / path;
				...
				else if (ext == ".gltf" || ext == ".glb" || ext == ".fbx" || ext == ".obj" || ext == ".dae")
				{
					...
					Ref<Model> model = AssetManager::GetModel(droppedPath.string());
					if (model && !model->GetSubmeshes().empty())
					{
						Entity entity = m_ActiveScene->CreateEntity(droppedPath.stem().string());
						entity.AddComponent<MeshRendererComponent>(model, m_MeshShader);
						m_SceneHierarchyPanel.SetSelectedEntity(entity);
						VIR_CORE_INFO("Dropped model loaded: {0}", droppedPath.string());
					}
					else
					{
						VIR_WARN("Could not load dropped model: {0}", droppedPath.string());
					}
				}
				else
				{
					VIR_WARN("Unsupported dropped file type: {0}", droppedPath.string());
				}
	...
	void Editor3DLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(VIR_BIND_EVENT_FN(Editor3DLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(VIR_BIND_EVENT_FN(Editor3DLayer::OnMouseButtonPressed));
	}
	...
	void Editor3DLayer::OpenScene()
	{
		std::string filepath = FileDialogs::OpenFile("Vireo Scene (*.vireo)\0*.vireo\0");
		if (!filepath.empty())
		{
			OpenScene(filepath);
		}
	...
	void Editor3DLayer::OpenScene(const std::filesystem::path& path)
	{
		if (m_SceneState != SceneState::Edit)
			OnSceneStop();

		if (path.extension().string() != ".vireo")
		{
			VIR_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	void Editor3DLayer::SaveSceneAs()
	{
		std::string filepath = FileDialogs::SaveFile("Vireo Scene (*.vireo)\0*.vireo\0");
		if (!filepath.empty())
		{
			/*SceneSerializer serializer(m_ActiveScene);
	...
	void Editor3DLayer::SerializeScene(Ref<Scene> scene, const std::filesystem::path& path)
	{
		SceneSerializer serializer(scene);
		serializer.Serialize(path.string());
	}

	void Editor3DLayer::OnScenePlay()
	{
	...
	void Editor3DLayer::OnSceneStop()
	{
	...
	void Editor3DLayer::OnDuplicateEntity()
	{
		if (m_SceneState != SceneState::Edit)
			return;

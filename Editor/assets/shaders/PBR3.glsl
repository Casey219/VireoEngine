...
void main() {
	if(any(isnan(v_WorldPos))){
		//o_Color = vec4(1.0, 0.0, 1.0, 1.0); // 洋红色：v_WorldPos 含 NaN
		return;
	}
	if (any(isnan(v_WorldPos)) || any(isnan(v_TBN[0])) || any(isnan(v_TBN[1])) || any(isnan(v_TBN[2]))) {
        //o_Color = vec4(1.0, 0.0, 1.0, 1.0); // 洋红色：输入数据含 NaN
        return;
    }
    ...
    // 检查插值后的 N 长度
    float lenN = length(N_raw);
    if (lenN < 0.001) {
        o_Color = vec4(1.0, 1.0, 0.0, 1.0); // 黄色：N 向量长度几乎为 0
        return;
    }
	vec3 N0 = normalize(N_raw);

    // 3. 检查 V 和 L
    vec3 V0 = normalize(u_CameraPos - v_WorldPos);
    vec3 L0 = normalize(u_LightPos.xyz - v_WorldPos);
    
    if (any(isnan(V0)) || any(isnan(L0))) {
        //o_Color = vec4(0.0, 1.0, 1.0, 1.0); // 青色：V 或 L 含 NaN
        return;
    }

    // 4. 检查 H
    vec3 H0 = normalize(V0 + L0);
    if (any(isnan(H0))) {
         //o_Color = vec4(1.0, 0.0, 0.0, 1.0); // 红色：H 含 NaN
         return;
    }
    ...
    if (u_HasMRMap == 1) {
        //vec3 mrSample = texture(u_MetallicRoughnessMap, v_TexCoord).rgb;
         //注意：这里绝对不要加 float 关键字，否则会产生局部遮蔽！
    ...
    metallic  = clamp(metallic, 0.02, 1.0);
